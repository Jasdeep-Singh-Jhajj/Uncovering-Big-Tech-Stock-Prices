...
grouped_data <- merged_data |>
  ...
  summarize(GainDays = compute_gain_days(close)) |>
  ungroup()

# Order the companies based on total gain days within each year
grouped_data <- grouped_data |>
  group_by(year) |>
  mutate(CumulativeGainDays = cumsum(GainDays)) |>
  mutate(Rank = rank(-CumulativeGainDays)) |>
  arrange(year, Rank) |>
  ungroup()
...
animated_plot <- ggplot(grouped_data) +
  geom_col(aes(x = CumulativeGainDays,
               y = fct_reorder(CompanyName, CumulativeGainDays),
               fill = CompanyName),
           width = 0.9) +
  geom_text(aes(x = CumulativeGainDays, y = CompanyName, label = CumulativeGainDays),
            hjust = -0.02, vjust = 0.5) +
  theme_minimal() +
  theme(axis.text.y = element_text(size = 10),
        axis.ticks.y = element_blank(),
        plot.margin = unit(c(1, 4, 3, 1), 'lines'),
        legend.position = "none",
        panel.grid.major=element_blank(),
        panel.grid.minor=element_blank(),
        panel.grid.major.x = element_line( size=.01, color="grey" )) +
  labs(
    x = "Cumulative Gain Days",
    y = NULL
  ) +
  scale_x_continuous(breaks = seq(0,2500, by = 500), limits = c(0, 2500)) +
  ggtitle('Cumulative Gain Days of Big Tech Companies: {closest_state}') +
