...
# Group data by year and company and calculate ranking
grouped_data <- merged_data |>
  group_by(year = year(date), CompanyName) |>
  summarize(ClosingPrice = mean(close)) |>
  ungroup() |>
  group_by(year) |>
  mutate(Rank = rank(ClosingPrice))# |>

clr <- c("#A2AAAD", "#FF0000", "#FF9900", "#1798c1", "#00BCEB", "#34A853", "#0530AD", 
         "#0068B5", "#0668E1", "#F14F21", "#E50914", "#76B900", "#C74634", "#E31937")

# Plot
animated_plot <- ggplot(grouped_data) +
  geom_col(aes(x = ClosingPrice,
               y = fct_reorder(CompanyName, Rank),
               fill = CompanyName),
           width = 0.9) +
  geom_text(aes(x = ClosingPrice, y = CompanyName, label = CompanyName),
            hjust = 1.25) +
  theme_minimal() +
  ...
  ggtitle('Yearwise Closing Price of Tech Comapnies: {closest_state}') +
